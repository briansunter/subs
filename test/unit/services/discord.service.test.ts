 ...
 * Tests PRODUCTION code (src/services/discord.ts) with mocked fetch
 */
...
describe("Discord Service - Unit Tests", () => {
  ...
  describe("sendDiscordNotification", () => {
    test("should send notification with correct payload", async () => {
      ...
      expect(fetchCalls[0]?.url).toBe(mockWebhookUrl);
    ...
    test("should skip notification when webhook URL not provided", async () => {
      await sendDiscordNotification({
        username: "Test Bot",
        content: "Test message",
      });
    ...
    test("should handle webhook error without throwing", async () => {
      const errorUrl = `${mockWebhookUrl}/error`;
